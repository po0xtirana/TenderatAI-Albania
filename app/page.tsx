...
function TenderTableRow({ record, onFeedback }: { record: TenderRecord; onFeedback: (relevant: boolean) => void }) {
  ...
  return <div className="table-row"><Link href={`/tenders/${encodeURIComponent(record.tender.id)}`} className="table-title"><strong>{record.tender.referenceNumber}</strong><span>{record.tender.contractObject}</span></Link><span className="authority-cell">{record.tender.contractingAuthority}</span><span className="amount-cell">{money(record.tender.limitFundAll)}</span><span className="deadline-cell"><b>{dateLabel(record.tender.submissionDeadline)}</b>{remainingDays != null && <small>{remainingDays <= 0 ? "I kaluar" : `${remainingDays} ditë`}</small>}</span><span className="match-cell"><span className={decisionClass(record.match.decision)}>{record.match.score}</span><small>{eligibility}</small></span><div className="row-actions"><button className={record.relevanceFeedback === true ? "active" : ""} aria-pressed={record.relevanceFeedback === true} onClick={() => onFeedback(true)} title="Shënoje relevant">✓</button><button className={record.relevanceFeedback === false ? "active negative" : ""} aria-pressed={record.relevanceFeedback === false} onClick={() => onFeedback(false)} title="Shënoje jo relevant">×</button></div></div>;
}
